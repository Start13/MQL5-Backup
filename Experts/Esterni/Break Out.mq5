...
//Copyright 2024, ALLAN MUNENE MUTIIRIA. #@Forex Algo-Trader. | https://youtube.com/@ForexAlgo-Trader? | 
//+------------------------------------------------------------------+ 
...
#property description "3. Incase of anything, contact developer via the link provided." "Hope you will enjoy the EA Logic.\n" 
#property description "*** HAPPY TRADING! ***" 
#property version "1.00" 

#include <Trade/Trade.mqh> // Include the tradelibrary 
...
input double tp_points = 500.0; // Take profit points Page 

int OnInit()
{ //--// Initialization code here (we don't initialize anything) //--

return(INIT_SUCCEEDED);}
void OnDeinit(const int reason){ //--// Deinitialization code here (we don't deinitialize anything) 
} 
...
TIME1_X1 = iTime(_Symbol,_Period,rangeBars); // Get the start time of the range Page 2/6
...
if (Ask > R_HighBreak_Prc){ // If the Ask price breaks the high breakout price Page 3/6
...
obj_Trade.Buy(0.01,_Symbol,Ask,Bid-sl_points*_Point,Bid+tp_points*_Point); return; // Exit the function 
} else if (Bid < R_LowBreak_Prc){ // If the Bid price breaks the low breakout price 
Print("SELL NOW"); // Print a message to sell 
isInRange = false; isRangeExist = false; // Reset range flags 
if (PositionsTotal() > 0){return;} // Exit the function 
obj_Trade.Sell(0.01,_Symbol,Bid,Ask+sl_points*_Point,Ask-tp_points*_Point); return; // Exit the function 
} 
...
if (Bid < PRICE2_Y2){ // If the Bid price is lower than the current low price 
...
//rangeName - name of the range object //| time1_x1 - start time of the range //| price1_y1 - high price of the range //| time2_x2 - end time of the range 
//| price2_y2 - low price of the range | | | | | | //+------------------------------------------------------------------+ 
void plotConsolidationRange(string rangeName,datetime time1_x1,double price1_y1, datetime time2_x2,double price2_y2)
{ 
...
else { // If the range object exists Page 5/6
