...
int OnInit()
  {
   ...
   PlotIndexSetString(4,PLOT_LABEL,"Zona resistenza alta");
   PlotIndexSetString(5,PLOT_LABEL,"Zona resistenza bassa");
   PlotIndexSetString(6,PLOT_LABEL,"Zona di supporto alta");
   PlotIndexSetString(7,PLOT_LABEL,"Zona di supporto bassa");
   PlotIndexSetString(8,PLOT_LABEL,"Forza della zona resistenza");
   PlotIndexSetString(9,PLOT_LABEL,"Forza della zona di supporto");
   PlotIndexSetString(10,PLOT_LABEL,"Prezzo all'interno della zona");
      ...
      if(Close[0]>=zone_lo[i] && Close[0]<zone_hi[i])
        {
            ...
            if(zone_alert_popups==true)
              {
               if(zone_type[i]==ZONE_SUPPORT)
                  Alert(Symbol()+" "+TFTS(timeframe)+": Entrato nella zona di supporto.");
               else
                  Alert(Symbol()+" "+TFTS(timeframe)+": Entrato nella zona di resistenza.");
              }
         ...
         if(zone_send_notification==true)
           {
            if(zone_type[i]==ZONE_SUPPORT)
               SendNotification(Symbol()+" "+TFTS(timeframe)+": Entrato nella zona di supporto.");
            else
               SendNotification(Symbol()+" "+TFTS(timeframe)+": Entrato nella zona di resistenza.");
           }
         return(true);
        }
     }
   ...
   for(int i=0; i<zone_count; i++)
     {
      ...
      string s;
      if(zone_type[i]==ZONE_SUPPORT)
         s=prefix+"S"+string(i)+" Forte=";
      else
         //name res
         s=prefix+"R"+string(i)+" Forte=";
      if(zone_strength[i]==ZONE_PROVEN)
         s=s+"Provato, conteggio dei test="+string(zone_hits[i]);
      else
         if(zone_strength[i]==ZONE_VERIFIED)
            s=s+"Verificato, conteggio test="+string(zone_hits[i]);
         else
