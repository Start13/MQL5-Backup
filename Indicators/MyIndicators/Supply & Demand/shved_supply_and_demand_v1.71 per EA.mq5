...
input string               string_prefix = "";                 // Change prefix to add multiple indicators to chart
...
void visualArray(double &arrayh[],double &arrayl[])
{
for(int x=0;x<ArraySize(arrayh);x++)
{
if(arrayh[x])Print(" x: ",x," High ",x,": ",arrayh[x]," Low ",x,": ",arrayl[x]);
}
}  
//+------------------------------------------------------------------+
//|                                                                  |
...
void resetArray()
{
for(int i=0;i<ArraySize(High_);i++){High_[i]=0;}
for(int i=0;i<ArraySize(Low_);i++){Low_[i]=0;}
}  