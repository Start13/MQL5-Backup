...
//Modified, 11/feb/2024, by jeanlouie, www.forexfactory.com/jeanlouie
// - conversion from mql4
...
//---- input parameters
input int       period = 18;
input bool      show_d1_swing = false;

input string    XABC_setting = "<<-- XABC Settings -->>";
input bool      XABC_enabled = true;
input datetime  XABC_spec_time = 0; //D'2008.12.1 00:00';
input string    XABC_font = "Arial";
input int       XABC_font_size = 16;
input color     XABC_font_color = White;
input color     XABC_ME_line_color = Yellow;
//---- constants
...
//---- indicators
   ...
   if (show_d1_swing)PlotIndexSetInteger(0,PLOT_DRAW_TYPE,DRAW_SECTION);
      //SetIndexStyle(0,DRAW_SECTION);
   else PlotIndexSetInteger(0,PLOT_DRAW_TYPE,DRAW_NONE);
      //SetIndexStyle(0,DRAW_NONE);
   SetIndexBuffer(0,d1_swing,INDICATOR_DATA);ArraySetAsSeries(d1_swing,true);   
   ...
   SetIndexBuffer(1,dn_swing,INDICATOR_DATA);ArraySetAsSeries(dn_swing,true);   
   //SetIndexEmptyValue(1, 0);
   PlotIndexSetDouble(1,PLOT_EMPTY_VALUE,0);
   //SetIndexLabel(1, period + "-period Swing");
   PlotIndexSetString(1,PLOT_LABEL,string(period) + "-period Swing");
   
   //SetIndexStyle(2,DRAW_SECTION);
   PlotIndexSetInteger(2,PLOT_DRAW_TYPE,DRAW_SECTION);
   SetIndexBuffer(2,XABC,INDICATOR_DATA);ArraySetAsSeries(XABC,true);   
   ...
   SetIndexBuffer(3,imean,INDICATOR_DATA);ArraySetAsSeries(imean,true);   
   //SetIndexLabel(3, "Impluse mean");
   PlotIndexSetString(3,PLOT_LABEL,"Impluse mean");

   //SetIndexStyle(4, DRAW_NONE);
   PlotIndexSetInteger(4,PLOT_DRAW_TYPE,DRAW_NONE);
   SetIndexBuffer(4, signals,INDICATOR_DATA);ArraySetAsSeries(signals,true);   
   //SetIndexLabel(4, "Signals");
   PlotIndexSetString(4,PLOT_LABEL,"Signals");
   
   SetIndexBuffer(5,d1_line_direct,INDICATOR_CALCULATIONS);ArraySetAsSeries(d1_line_direct,true);   
   SetIndexBuffer(6,dn_line_direct,INDICATOR_CALCULATIONS);ArraySetAsSeries(dn_line_direct,true);   
                ...
                const int &spread[])
{
   ...
   int   counted_bars= prev_calculated==0 ? 0 :prev_calculated-1;
   double shd_h, shd_l;
   int direct;
   ...
   int   i, j, n;
//----
   n = Bars(_Symbol,_Period) - counted_bars - 1;
   i = n;
   
   while (i>=0){
      if (i == Bars(_Symbol,_Period) - 1){
         ...
         switch (direct){
            case LINE_DIRECT_UP:
               ...
               if (d1_line_direct[i+1] == LINE_NO_DIRECT){
                  j = i+1;
                  while(d1_line_direct[j] == LINE_NO_DIRECT && j <= Bars(_Symbol,_Period)-1) j++;
   ...
   while (i>=0){
         ...
         switch (direct){
            case LINE_DIRECT_UP:
               ...
               if (dn_line_direct[i+1] == LINE_NO_DIRECT){
                  j = i+1;
                  while(dn_line_direct[j] == LINE_NO_DIRECT && j <= Bars(_Symbol,_Period)-1) j++;
      ...
      while(val_3 == 0 && j <= Bars(_Symbol,_Period)-1){
         if (dn_swing[j] > 0){
            if (val_0 == 0){
               val_0 = dn_swing[j];
            } else {
      ...
      GlobalVariableSet(string(period)+"d_Impluse_Mean", impulse_mean);
      GlobalVariableSet(string(period)+"d_Impluse_Count", impulse_count);
...
int dn_direct(int idx, int n){
   ...
   while(val_2 == 0 && j <= Bars(_Symbol,_Period)-1){
      if (d1_swing[j] > 0){
         if (val_1 <= 0){
            val_1 = d1_swing[j];
         } else {
...
void show_XABC(){
   ...
   align = (int)ChartGetInteger(0,CHART_WIDTH_IN_BARS)/20;// * 10 / 100;
      
   if (X_pt < A_pt){
      ME = MathMax(XA_10pc, AB_20pc);
      ObjectCreate(0,"XABC_PSZ", OBJ_TREND, 0, A_dt, A_pt-ABdiv8, Time[0], A_pt-ABdiv8);ObjectSetInteger(0,"XABC_PSZ",OBJPROP_RAY_RIGHT,true);
      ObjectCreate(0,"XABC_PSZ_Label", OBJ_TEXT, 0, Time[0]+PeriodSeconds()*align, A_pt-ABdiv8);
      ObjectCreate(0,"XABC_PBZ", OBJ_TREND, 0, B_dt, B_pt+ABdiv8, Time[0], B_pt+ABdiv8);ObjectSetInteger(0,"XABC_PBZ",OBJPROP_RAY_RIGHT,true);
      ObjectCreate(0,"XABC_PBZ_Label", OBJ_TEXT, 0, Time[0]+PeriodSeconds()*align, B_pt+ABdiv8);
      ObjectSetText("XABC_PSZ_Label", "Primary Sell Zone @"+DoubleToString(A_pt-ABdiv8,_Digits), 8, XABC_font ,XABC_ME_line_color);
      ObjectSetText("XABC_PBZ_Label", "Primary Buy Zone @"+DoubleToString(B_pt+ABdiv8,_Digits), 8, XABC_font ,XABC_ME_line_color);
      pop_XABC_signals(X_pt, A_pt, A_bar, B_pt, B_bar, C_pt, C_bar, A_pt+ME, B_pt-ME, B_pt+ABdiv8, A_pt-ABdiv8, total_XABC_swing_pt);
   } else {
      ME = -MathMax(XA_10pc, AB_20pc);
      ObjectCreate(0,"XABC_PBZ", OBJ_TREND, 0, A_dt, A_pt+ABdiv8, Time[0], A_pt+ABdiv8);ObjectSetInteger(0,"XABC_PBZ",OBJPROP_RAY_RIGHT,true);
      ObjectCreate(0,"XABC_PBZ_Label", OBJ_TEXT, 0, Time[0]+PeriodSeconds()*align, A_pt+ABdiv8);
      ObjectCreate(0,"XABC_PSZ", OBJ_TREND, 0, B_dt, B_pt-ABdiv8, Time[0], B_pt-ABdiv8);ObjectSetInteger(0,"XABC_PSZ",OBJPROP_RAY_RIGHT,true);
      ObjectCreate(0,"XABC_PSZ_Label", OBJ_TEXT, 0, Time[0]+PeriodSeconds()*align, B_pt-ABdiv8);
      ObjectSetText("XABC_PSZ_Label", "Primary Sell Zone @"+DoubleToString(B_pt-ABdiv8,_Digits), 8, XABC_font ,XABC_ME_line_color);
      ObjectSetText("XABC_PBZ_Label", "Primary Buy Zone @"+DoubleToString(A_pt+ABdiv8,_Digits), 8, XABC_font ,XABC_ME_line_color);
      pop_XABC_signals(X_pt, A_pt, A_bar, B_pt, B_bar, C_pt, C_bar, B_pt-ME, A_pt+ME, A_pt+ABdiv8, B_pt-ABdiv8, total_XABC_swing_pt);
   }
   
   ObjectCreate(0,"XABC_A", OBJ_TREND, 0, A_dt, A_pt, Time[0], A_pt);   ObjectSetInteger(0,"XABC_A",OBJPROP_RAY_RIGHT,true);
   ObjectCreate(0,"XABC_A_Label", OBJ_TEXT, 0, Time[0]+PeriodSeconds()*align, A_pt);
   ObjectSetText("XABC_A_Label", "A Boundary @"+DoubleToString(A_pt, _Digits), 8, XABC_font ,XABC_ME_line_color);

   ObjectCreate(0,"XABC_A_ME", OBJ_TREND, 0, A_dt, A_pt+ME, Time[0], A_pt+ME);ObjectSetInteger(0,"XABC_A_ME",OBJPROP_RAY_RIGHT,true);
   ObjectCreate(0,"XABC_A_ME_Label", OBJ_TEXT, 0, Time[0]+PeriodSeconds()*align, A_pt+ME);
   ObjectSetText("XABC_A_ME_Label", "Maximum Extension @"+DoubleToString(A_pt+ME, _Digits), 8, XABC_font ,XABC_ME_line_color);

   ObjectCreate(0,"XABC_B", OBJ_TREND, 0, B_dt, B_pt, Time[0], B_pt);ObjectSetInteger(0,"XABC_B",OBJPROP_RAY_RIGHT,true);
   ObjectCreate(0,"XABC_B_Label", OBJ_TEXT, 0, Time[0]+PeriodSeconds()*align, B_pt);
   ObjectSetText("XABC_B_Label", "B Boundary @"+DoubleToString(B_pt,_Digits), 8, XABC_font ,XABC_ME_line_color);

   ObjectCreate(0,"XABC_B_ME", OBJ_TREND, 0, B_dt, B_pt-ME, Time[0], B_pt-ME);ObjectSetInteger(0,"XABC_B_ME",OBJPROP_RAY_RIGHT,true);
   ObjectCreate(0,"XABC_B_ME_Label", OBJ_TEXT, 0, Time[0]+PeriodSeconds()*align, B_pt-ME);
   ObjectSetText("XABC_B_ME_Label", "Maximum Extension @"+DoubleToString(B_pt-ME, _Digits), 8, XABC_font ,XABC_ME_line_color);

   ObjectSetInteger(0,"XABC_PSZ", OBJPROP_COLOR, XABC_ME_line_color);
   ObjectSetInteger(0,"XABC_PBZ", OBJPROP_COLOR, XABC_ME_line_color);
   ObjectSetInteger(0,"XABC_A", OBJPROP_COLOR, XABC_ME_line_color);
   ObjectSetInteger(0,"XABC_B", OBJPROP_COLOR, XABC_ME_line_color);
   ObjectSetInteger(0,"XABC_A_ME", OBJPROP_COLOR, XABC_ME_line_color);
   ObjectSetInteger(0,"XABC_B_ME", OBJPROP_COLOR, XABC_ME_line_color);

   ObjectSetInteger(0,"XABC_PSZ", OBJPROP_STYLE, STYLE_DOT);
   ObjectSetInteger(0,"XABC_PBZ", OBJPROP_STYLE, STYLE_DOT);
   ObjectSetInteger(0,"XABC_A", OBJPROP_STYLE, STYLE_DOT);
   ObjectSetInteger(0,"XABC_B", OBJPROP_STYLE, STYLE_DOT);
   ObjectSetInteger(0,"XABC_A_ME", OBJPROP_STYLE, STYLE_DOT);
   ObjectSetInteger(0,"XABC_B_ME", OBJPROP_STYLE, STYLE_DOT);
}
...
void catch_swing_pt(){
   ...
   while((count <= 3 || MathAbs(sw_pt_1 - sw_pt_2) < imp_mean) && i <= Bars(_Symbol,_Period)-1){
      if(dn_swing[i] > 0){
         if (sw_pt_1 == 0){
            sw_pt_1 = dn_swing[i];
            sw_pt_2 = dn_swing[i];
...
void catch_swing_pt_ex(){
   ...
   while(!imp_mean_hit && i <= Bars(_Symbol,_Period)-1){
      if(dn_swing[i] > 0){
         if (sw_pt_1 == 0){
            sw_pt_1 = dn_swing[i];
            sw_pt_2 = dn_swing[i];
...
void clear_label(){
   ...
   for (int i=0; i<=num_label; i++){
      strlabel = "XABC_label_" + string(i);
      ObjectDelete(0,strlabel);
   }
   ObjectDelete(0,"XABC_A");
   ObjectDelete(0,"XABC_A_ME");
   ObjectDelete(0,"XABC_B");
   ObjectDelete(0,"XABC_B_ME");
   ObjectDelete(0,"XABC_PBZ");
   ObjectDelete(0,"XABC_PSZ");

   ObjectDelete(0,"XABC_A_Label");
   ObjectDelete(0,"XABC_B_Label");
   ObjectDelete(0,"XABC_A_ME_Label");
   ObjectDelete(0,"XABC_B_ME_Label");
   ObjectDelete(0,"XABC_PBZ_Label");
   ObjectDelete(0,"XABC_PSZ_Label");
}
...
void ObjectSetText(string name, string text, int fsize, string font, color clr)
{
